...
HTTPSource = (function(_super) {
  ...
  function HTTPSource(url, opts) {
    ...
    this.chunkSize = this.opts.chunkSize || 1 << 20;
    this.sequentialHTTPErrorThreshold = this.opts.sequentialHTTPErrorThreshold || 3;
    this.sequentialHTTPErrorCount = 0;
    this.inflight = false;
    this.reset();
  }
  ...
  HTTPSource.prototype.loop = function() {
    ...
    this.xhr.onload = (function(_this) {
      return function(event) {
        ...
        if (_this.sequentialHTTPErrorCount >= _this.sequentialHTTPErrorThreshold) {
          return _this.emit('error', "Unable to process an HTTP request. Got error " + _this.sequentialHTTPErrorCount + " times in a row.");
        }
        if (event.target.status >= 400) {
          ++_this.sequentialHTTPErrorCount;
        } else {
