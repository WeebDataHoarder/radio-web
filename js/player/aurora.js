...
},{"./asset":1,"./core/base":4,"./core/bitstream":5,"./core/buffer":6,"./core/bufferlist":7,"./core/events":8,"./core/stream":9,"./core/underflow":10,"./decoder":11,"./demuxer":14,"./device":20,"./devices/resampler":22,"./filter":24,"./filters/balance":25,"./filters/volume":26,"./player":27,"./sources/buffer":31,"./sources/node/file":29,"./sources/node/http":30}],4:[function(require,module,exports){
var Base,
  __hasProp = {}.hasOwnProperty,
  __extends = function(child, parent) { for (var key in parent) { if (__hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; },
...
AudioDevice = (function(_super) {
  ...
  function AudioDevice(sampleRate, channels, options) {
  ...
  AudioDevice.prototype.start = function() {
    ...
    this.playing = true;
    if (this.device == null) {
      this.device = AudioDevice.create(this.sampleRate, this.channels, this.options);
    }
    if (!this.device) {
  ...
  AudioDevice.register = function(device, weight) {
    return devices.push([device, weight || 5]);
  };

  AudioDevice.create = function(sampleRate, channels, options) {
    var device;
    device = this.device();
    return device && new device(sampleRate, channels, options);
  };

  AudioDevice.device = function() {
    var device, _devices, _i, _len;
    _devices = devices.sort(function(a, b) {
      return a[1] - b[1];
    }).map(function(element) {
      return element[0];
    });
    for (_i = 0, _len = _devices.length; _i < _len; _i++) {
      device = _devices[_i];
      if (device.supported) {
        return device;
      }
  ...
  AudioDevice.deviceSampleRate = function() {
    var device;
    device = this.device();
    return typeof device.deviceSampleRate === "function" ? device.deviceSampleRate() : void 0;
  };
...
Player = (function(_super) {
  ...
  function Player(asset, options) {
    ...
    this.asset.on('decodeStart', (function(_this) {
      return function() {
        _this.queue = new Queue(_this.asset, _this.options.queue);
...
//# sourceMappingURL=/js/player/aurora.js.map
