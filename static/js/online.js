...
function initWebsite() {
    document.querySelector("#radio-quality").addEventListener('change', function () {
        ...
        for(const aStream of compatibleAudioStreams){
            if (aStream.id == this.value) {
                audioStream = aStream;
                break;
            }
        }
...
function updateQueueData(res) {
    ...
    if (res.length === 0) {
        ...
        document.querySelector("#radio-queue").innerHTML = "";
        for(const r of res){
            document.querySelector("#radio-queue").append(createQueueEntry(r, queueDuration));
            if (queueDuration !== null) {
                queueDuration += r.duration;
            }
        }
    }
}
...
audioStream = null;
if (connection && (connection.type === "cellular" || connection.type === "bluetooth" || connection.effectiveType === "slow-2g" || connection.effectiveType === "2g" || connection.effectiveType === "3g" || connection.saveData)) {
    //better data usage
